...
COPY INTO RAW_RESTAURANTS (raw_json, _file_name)
FROM (
...
COPY INTO RAW_AGENTS (
    agent_id, agent_name, phone_number, city, vehicle_type,
    joining_date, agent_rating, availability_status,
    updated_at, _file_name
)
FROM (
...
UNION ALL SELECT 'RAW_RESTAURANTS',  COUNT(*) FROM RAW_RESTAURANTS
UNION ALL SELECT 'RAW_AGENTS',       COUNT(*) FROM RAW_AGENTS
ORDER BY tbl;